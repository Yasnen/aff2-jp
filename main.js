...
Hooks.on("chatMessage", (html, content, msg) => {
    ...
    if (aff) {
        ...
        let regExp;
        regExp = /#+\s*(.+)/g;
        let comment = content.match(regExp);
            ...
            if (commands[1] === "ft") {
                ...
                if (commands[2] !== undefined) {
                    if (commands[2].substr(0, 1) === '[') {
                        ...
                        if (commands[3] !== undefined && isFinite(commands[3])) {
                            mod = commands[3]
                        ...
                        if (mod === "") {
                            msg.content = `<b>結果：${retVal}</b> = 1d6(${dice + 1}) ${commands[2]}`;                                                
                        } else {
                            msg.content = `<b>結果：${retVal}</b> = 1d6(${dice + 1}) ${mod} ${commands[2]}`;
                        }

                        if(comment != null) {
                            msg.content += ` ${comment} </ br>`;
                        }       
